...
```{r Importing GIS Watershed Attr}
...
```{r Importing GIS Pour attributes}
...
```{r Extract Site and Transect Layers}
...
```{r XS and GIS Levels Comparison}
...
```
Data look good. We can now advance with merging datasets
...
```{r Split site names back to site and transect}
XS_Change <- XS_Change %>%
...
```{r Merging Site Datasets - Calculating means for Class and Site}
XS_ws_site <- XS_Change %>% #Calculating mean based on average of Left and Right sides of Channel
  group_by(Site,Class) %>%
...
```{r Merging Transect Datasets - Calculating means for Class and Site*Transect}
...
```{r}
XS_ws_transect %>%
  ggplot(aes(x=contr_area.x,y=net.rate))+
  geom_point()+
  facet_wrap(~Class)+
  geom_smooth()
```
...
```{r}
tile_ws_transect %>%
  ggplot(aes(x=log(Dist_from_thalweg*elev_above_thalweg),y=deposit_mm_yr))+
  geom_point()+
  facet_wrap(~strahler)+
  geom_smooth(method=lm)

fit<- lm(data=tile_ws_transect, log(deposit_mm_yr+1)~Dist_from_thalweg+elev_above_thalweg+strahler+area_imper)
summary(fit)
step(fit)
summary(fit)
```
